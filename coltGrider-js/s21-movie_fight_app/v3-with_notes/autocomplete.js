const createAutocomplete = ({root, renderOption, onOptionSelect, inputValue, fetchData}) => {
    //specify where to render the autocomplete on the webpage
    ...
    `;
    //look for these elements inside root instead of the entire document
    ...
    const onInput = debounce(async event => {
        ...
        //fetchData() returns the data from api. Assign this data to variable items
        ...
        //if no items is returned, close the dropdown and exit the function
        if (!items.length) {
            dropdown.classList.remove('is-active');
            return;
        }
    
        //clear out the list of results before fetching a new query
        resultsWrapper.innerHTML = '';
        dropdown.classList.add('is-active');
        //iterate thru the items array, create a new div element and pass in details of each item into it
        for (let item of items) {
            ...
            //do a followup request to get detailed info of the selected item
